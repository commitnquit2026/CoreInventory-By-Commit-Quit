...
import { Plus, Printer, Check, X } from 'lucide-react'
...
export default function DeliveriesPage() {
      ...
      setDeliveries([
        {
          ...
          items: [
            { product_id: 1, product_name: 'Desk', quantity: 5 },
          ],
        },
        {
          id: 2,
          ...
          created_date: '2026-03-14',
          items: [
            { product_id: 2, product_name: 'Chair', quantity: 10 },
          ],
        },
        {
          id: 3,
          ...
          items: [
            { product_id: 1, product_name: 'Desk', quantity: 3 },
          ],
        },
      ])
    } catch (e) {
  ...
  async function handleSaveDelivery() {
    ...
    try {
      ...
      const newDelivery = {
        ...
        delivery_id: `DEL${String(deliveries.length + 1).padStart(3, '0')}`,
        ...form,
        status: 'Draft',
  ...
  const getNextStatus = (status) => {
    const flow = { 'Draft': 'Waiting', 'Waiting': 'Ready', 'Ready': 'Done' }
    return flow[status]
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-heading text-2xl font-semibold text-slate-900">Deliveries</h1>
        <p className="text-sm text-slate-500">Manage outbound deliveries and stock allocation.</p>
      </div>
            ...
            <tbody>
              {deliveries.map(delivery => (
                <tr key={delivery.id} className="border-b border-slate-200 hover:bg-slate-50">
                  <td className="py-3 px-3 font-medium">{delivery.delivery_id}</td>
                  <td className="py-3 px-3">{delivery.created_date}</td>
                  <td className="py-3 px-3">{delivery.items.length}</td>
                  ...
                  <td className="py-3 px-3 flex gap-2">
                    {delivery.status !== 'Done' && (
                      <button
                        ...
                        title={`Move to ${getNextStatus(delivery.status)}`}
                      >
